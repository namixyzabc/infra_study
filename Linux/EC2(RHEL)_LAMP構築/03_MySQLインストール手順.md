...
### 1. リポジトリの追加と確認
OS（AlmaLinuxなど）のデフォルトのリポジトリにはMySQLが含まれていないため、**MySQL公式サイトが提供するリポジトリ**を導入します。
...
### 5. スロークエリログの設定（推奨設定）
実行に時間がかかり負荷の原因となるSQLを特定するため、**スロークエリログ**の出力設定を行うことが推奨。
*   **設定ファイルの編集**: `/etc/my.cnf` を開き、以下の内容を追記します:
    *   `slow_query_log = 1`（有効化）
    *   `long_query_time = 1`（1秒以上かかるクエリを記録。デフォルトの10秒は長すぎる。１秒＝1000ミリ秒）
